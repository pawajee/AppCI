...
qrcode.decode = function(canvas){
    ...
    try {
        response = qrcode.process(context);
    } catch(e) {
        return {error:e}
    }
...
qrcode.process = function(ctx) {
    ...
    return { data: qrcode.decode_utf8(str), time: time };
...
ds.sliceSize = ctor.sliceSize || 2900;
ds.baseInterval = ctor.baseInterval || 75;
...
function Slicer(data) { // jshint ignore:line
    ...
    return {
        ...
        nextSlice: function() {
			...
			if(!!window.__getSelectedSlice)
			{
				currentSlice=__getSelectedSlice(currentSlice);
			}
			if(!!window.__onNextSlice){
				__onNextSlice(currentSlice,slices.length);
			}
...
function Video(videoElement, canvas) { // jshint ignore:line
    ...
    async function start(source) {
		...
		if(navigator.mediaDevices){
			navigator.mediaDevices
					.getDisplayMedia().then(videoSuccess);
		}
    ...
    function videoSuccess(newVideoStream) {
        ...
        */
        ...
        if(!videoElement.srcObject &&  !!videoElement.mozSrcObject) {
            //console.log('mozilla');
